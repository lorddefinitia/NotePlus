...
void Gripper::CalcRectToScreen(HWND hWnd, RECT *rc)  {
		ClientRectToScreenRect(hWnd, rc);
		ShrinkRcToSize(rc);
	};

void Gripper::CalcRectToClient(HWND hWnd, RECT *rc)  {
		ScreenRectToClientRect(hWnd, rc);
		ShrinkRcToSize(rc);
	};

void Gripper::ShrinkRcToSize(RECT *rc)  {
		rc->right	-= rc->left;
		rc->bottom	-= rc->top;
	};

void Gripper::DoCalcGripperRect(RECT* rc, RECT rcCorr, POINT pt)  {
		if ((rc->left + rc->right) < pt.x)
			rc->left = pt.x - 20;
