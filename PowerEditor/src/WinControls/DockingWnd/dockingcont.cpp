...
BOOL CALLBACK DockingCont::run_dlgProc(UINT Message, WPARAM wParam, LPARAM lParam)
{
	switch (Message) 
	{
		...
		case WM_INITDIALOG:
		{
			...
			//FIXME fncpntr
			_hDefaultCaptionProc = reinterpret_cast<WNDPROC>(::SetWindowLongPtr(_hCaption, GWLP_WNDPROC, (void*)&wndCaptionProc));

			// intial subclassing of tab
			::SetWindowLongPtr(_hContTab, GWLP_USERDATA, (LONG_PTR)this);
			_hDefaultTabProc = reinterpret_cast<WNDPROC>(::SetWindowLongPtr(_hContTab, GWLP_WNDPROC, (void*)&wndTabProc));
...
HWND DockingCont::getTabWnd()  {
		return _hContTab;
	};

HWND DockingCont::getCaptionWnd()  { 		if (_isFloating == false)
			return _hCaption;
		else
			return _hSelf;
	};
...
bool DockingCont::isFloating()  {
		return _isFloating;
	}

INT DockingCont::getElementCnt()  {
		return _vTbData.size();
	}

BOOL DockingCont::startMovingFromTab()  {
...
void DockingCont::setTabStyle(const BOOL & bDrawOgLine)  {
...
CALLBACK DockingCont::wndCaptionProc(HWND hwnd, UINT Message, WPARAM wParam, LPARAM lParam)  {
		return (((DockingCont *)(::GetWindowLongPtr(hwnd, GWL_USERDATA)))->runProcCaption(hwnd, Message, wParam, lParam));
	};

CALLBACK DockingCont::wndTabProc(HWND hwnd, UINT Message, WPARAM wParam, LPARAM lParam)  {
		return (((DockingCont *)(::GetWindowLongPtr(hwnd, GWL_USERDATA)))->runProcTab(hwnd, Message, wParam, lParam));
	};

INT  DockingCont::removeTab(tTbData* pTbData)  {
