...
char * ByteArray::getPointer() const  {return _pBytes;};
size_t ByteArray::getLength() const  {return _length;};
void ClipboardHistoryPanel::init(HINSTANCE hInst, HWND hPere, ScintillaEditView **ppEditView)  {
		DockingDlgInterface::init(hInst, hPere);
		_ppEditView = ppEditView;
	};

void ClipboardHistoryPanel::display(bool toShow = true) const  {
        DockingDlgInterface::display(toShow);
    };

void ClipboardHistoryPanel::setParent(HWND parent2set) {
        _hParent = parent2set;
    };
