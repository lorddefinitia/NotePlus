...
bool Shortcut::isValid() const  { //valid should only be used in cases where the shortcut isEnabled().		
    if (_keyCombo._key == 0)
...
bool Shortcut::isEnabled() const  {	//true if _keyCombo != 0, false if _keyCombo == 0, in which case no accelerator should be made		return (_keyCombo._key != 0);
	};

generic_string Shortcut::toMenuItemString() const  {					//generic_string suitable for menu
...
const TCHAR * Shortcut::getName() const  {
		return _name;
	};

const TCHAR * Shortcut::getMenuName() const  {
		return _menuName;
	}

long CommandShortcut::getID() const  {return _id;};
void CommandShortcut::setID(unsigned long id)  { _id = id;};
long ScintillaKeyMap::getScintillaKeyID() const  {return _scintillaKeyID;};
int ScintillaKeyMap::getMenuCmdID() const  {return _menuCmdID;};
...
//bool Window::isPlayable() const
//bool Window::isValid() 

const TCHAR * PluginCmdShortcut::getModuleName() const  {return _moduleName.c_str();};
int PluginCmdShortcut::getInternalID() const  {return _internalID;};
long PluginCmdShortcut::getID() const  {return _id;};
...
HACCEL Accelerator::getAccTable() const  {return _hAccTable;};
