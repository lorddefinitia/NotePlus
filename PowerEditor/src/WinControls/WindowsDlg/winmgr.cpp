...
WINRECT* WINRECT::Prev()			{ return prev; }
WINRECT* WINRECT::Next()			{ return next; }
WINRECT* WINRECT::Children()		{ return IsGroup() ? this+1 : NULL; }
WINRECT* WINRECT::Parent();
WORD WINRECT::GetFlags()			{ return flags; }
WORD WINRECT::SetFlags(WORD f)	{ return flags=f; }
LONG WINRECT::GetParam()			{ return param; }
LONG WINRECT::SetParam(LONG p)	{ return param=p; }
UINT WINRECT::GetID()				{ return nID; }
UINT WINRECT::SetID(UINT id)		{ return nID=id; }
RECT& WINRECT::GetRect()					{ return rc; }
void WINRECT::SetRect(const RECT& r)	{ rc = r; }
WORD WINRECT::Type() const	 { return flags & WRCF_TYPEMASK; }
WORD WINRECT::GroupType() const	{ return flags & WRCF_GROUPMASK; }
BOOL WINRECT::IsGroup() const		{ return GroupType() && GroupType()!=WRCF_ENDGROUP; }
BOOL WINRECT::IsEndGroup() const { return flags==0 || flags==WRCF_ENDGROUP; }
BOOL WINRECT::IsEnd() const		{ return flags==0; }
BOOL WINRECT::IsWindow() const	{ return nID>0; }
BOOL WINRECT::IsRowGroup()	const { return (flags & WRCF_GROUPMASK)==WRCF_ROWGROUP; }
void WINRECT::SetHeight(LONG h)	{ rc.bottom = rc.top + h; }
void WINRECT::SetWidth(LONG w)	{ rc.right = rc.left + w; }
LONG WINRECT::GetHeightOrWidth(BOOL bHeight) const {
	return bHeight ? RectHeight(rc) : RectWidth(rc);
}
void WINRECT::SetHeightOrWidth(LONG horw, BOOL bHeight) {
	bHeight ? SetHeight(horw) : SetWidth(horw);
}
BOOL WINRECT::HasToFitSize() { return param != 0; }
SIZE WINRECT::GetToFitSize() { SIZE sz = {LOWORD(param),HIWORD(param)}; return sz; }
void WINRECT::SetToFitSize(SIZE sz)	{ param = MAKELONG(sz.cx,sz.cy); }

// calc layout using client area as total area
void CWinMgr::CalcLayout(HWND hWnd) {
	assert(hWnd);
	RECT rcClient;
...
void CWinMgr::CalcLayout(int cx, int cy, HWND hWnd=NULL) {
	RECT rc = {0,0,cx,cy};
	CalcLayout(rc, hWnd);
}

// calc layout using given rect as total area
void CWinMgr::CalcLayout(RECT rcTotal, HWND hWnd=NULL) {
	assert(m_map);
	m_map->SetRect(rcTotal);
	CalcGroup(m_map, hWnd);
}

void CWinMgr::MoveRect(int nID, POINT ptMove, HWND pParentWnd) {
	MoveRect(FindRect(nID), ptMove, pParentWnd);
}

RECT CWinMgr::GetRect(UINT nID) { return FindRect(nID)->GetRect(); }
void CWinMgr::SetRect(UINT nID, const RECT& rc) { FindRect(nID)->SetRect(rc); }
