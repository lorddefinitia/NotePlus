...
void ViewZoneDlg::destroy()  {
};

void ViewZoneDlg::drawZone(long hY, long lY)  {
		_higherY = hY;
		_lowerY = lY;
		::InvalidateRect(_viewZoneCanvas, NULL, TRUE);
	};

int ViewZoneDlg::getViewerHeight() const  {
		return (_lowerY - _higherY);
	};

int ViewZoneDlg::getCurrentCenterPosY() const  {
		return (_lowerY - _higherY)/2 + _higherY;
	};

void DocumentMap::create(tTbData * data, bool isRTL = false)  {
		DockingDlgInterface::create(data, isRTL);
		data->pszAddInfo = id4dockingCont.c_str();
	};

void DocumentMap::init(HINSTANCE hInst, HWND hPere, ScintillaEditView **ppEditView)  {
		DockingDlgInterface::init(hInst, hPere);
		_ppEditView = ppEditView;
	};

void DocumentMap::display(bool toShow = true) const  {
        DockingDlgInterface::display(toShow);
		_vzDlg.display();
    };

void DocumentMap::setParent(HWND parent2set) {
        _hParent = parent2set;
    };

void DocumentMap::vzDlgDisplay(bool toShow = true)  {
