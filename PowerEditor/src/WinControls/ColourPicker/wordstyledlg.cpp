...
COLORREF ColourStaticTextHooker::setColour(COLORREF colour2Set)  {
		COLORREF oldColour = _colour;
		_colour = colour2Set;
		return oldColour;
	};

void ColourStaticTextHooker::hookOn(HWND staticHandle)  {
		::SetWindowLongPtr(staticHandle, GWLP_USERDATA, (LONG_PTR)this);
		//FIXME fncpntr
		_oldProc = (WNDPROC)::SetWindowLongPtr(staticHandle, GWLP_WNDPROC, (void*)&staticProc);
	};

CALLBACK ColourStaticTextHooker::staticProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam) {
...
void WordStyleDlg::init(HINSTANCE hInst, HWND parent)	 {
        Window::init(hInst, parent);
	};

void WordStyleDlg::doDialog(bool isRTL = false)  {
    	if (!isCreated())
		{
