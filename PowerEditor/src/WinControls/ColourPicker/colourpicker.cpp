...
void ColourPicker::init(HINSTANCE hInst, HWND parent)
{
    ...
    //FIXME fncpntr
	_buttonDefaultProc = reinterpret_cast<WNDPROC>(::SetWindowLongPtr(_hSelf, GWLP_WNDPROC, (void*)&staticWinProc));	 
...
void ColourPicker::setColour(COLORREF c) {
    _currentColour = c;
};

COLORREF ColourPicker::getColour() const {return _currentColour;};
bool ColourPicker::isEnabled() {return _isEnabled;};
void ColourPicker::setEnabled(bool enabled) {_isEnabled = enabled;};
static LRESULT CALLBACK ColourPicker::staticWinProc(HWND hwnd, UINT Message, WPARAM wParam, LPARAM lParam) {
