...
BOOL CALLBACK DefaultNewDocDlg::run_dlgProc(UINT Message, WPARAM wParam, LPARAM)
{
	...
	switch (Message) 
	{
		case WM_INITDIALOG :
		{
			...
			//FIXME NULL?-> 0
			_customLenVal.create(::GetDlgItem(_hSelf, IDC_CUSTOMIZELENGTHVAL_STATIC), 0);
...
void MarginsDlg::destroy()  {
		_verticalEdgeLineNbColVal.destroy();
	};

void DefaultNewDocDlg::destroy()  {
		_nbHistoryVal.destroy();
		_customLenVal.destroy();
	};

void DefaultNewDocDlg::makeOpenAnsiAsUtf8(bool doIt) {
		if (!doIt)
...
void PreferenceDlg::doDialog(bool isRTL = false)  {
    	if (!isCreated())
		{
