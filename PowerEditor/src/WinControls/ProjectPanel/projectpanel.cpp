...
void ProjectPanel::display(bool toShow = true) const  {
        DockingDlgInterface::display(toShow);
    };

void ProjectPanel::setParent(HWND parent2set) {
        _hParent = parent2set;
    };

void ProjectPanel::setWorkSpaceFilePath(const TCHAR *projectFileName) {
		_workSpaceFilePath = projectFileName;
	};

const TCHAR * ProjectPanel::getWorkSpaceFilePath() const  {
		return _workSpaceFilePath.c_str();
	};

bool ProjectPanel::isDirty() const  {
		return _isDirty;
	};

void FileRelocalizerDlg::init(HINSTANCE hInst, HWND parent) {
		Window::init(hInst, parent);
	};

void FileRelocalizerDlg::destroy()  {
    };

generic_string FileRelocalizerDlg::getFullFilePath()  {
