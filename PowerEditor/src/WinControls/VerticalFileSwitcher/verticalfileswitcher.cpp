...
void VerticalFileSwitcher::init(HINSTANCE hInst, HWND hPere, HIMAGELIST hImaLst)  {
		DockingDlgInterface::init(hInst, hPere);
		_hImaLst = hImaLst;
	};

void VerticalFileSwitcher::display(bool toShow = true) const  {
        DockingDlgInterface::display(toShow);
    };

void VerticalFileSwitcher::setParent(HWND parent2set) {
        _hParent = parent2set;
    };

int VerticalFileSwitcher::newItem(int bufferID, int iView) {
		return _fileListView.newItem(bufferID, iView);
	};

int VerticalFileSwitcher::closeItem(int bufferID, int iView) {
		return _fileListView.closeItem(bufferID, iView);
	};

void VerticalFileSwitcher::activateItem(int bufferID, int iView)  {
		_fileListView.activateItem(bufferID, iView);
	};

void VerticalFileSwitcher::setItemIconStatus(int bufferID)  {
		_fileListView.setItemIconStatus(bufferID) ;
	};

generic_string VerticalFileSwitcher::getFullFilePath(size_t i) const  {
