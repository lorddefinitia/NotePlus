...
void TaskList::init(HINSTANCE hInst, HWND parent, HIMAGELIST hImaLst, int nbItem, int index2set)
{
    ...
    //FIXME HIGH PRIORITY, VALID FUNCTION CALL
...
static LRESULT CALLBACK TaskList::staticProc(HWND hwnd, UINT Message, WPARAM wParam, LPARAM lParam) {
	return (((TaskList *)(::GetWindowLongPtr(hwnd, GWL_USERDATA)))->runProc(hwnd, Message, wParam, lParam));
};

int TaskList::getCurrentIndex() const {return _currentIndex;}

HIMAGELIST TaskList::getImgLst() const {
	return ListView_GetImageList(_hSelf, LVSIL_SMALL);
};

HFONT TaskList::GetFontSelected() {return _hFontSelected;}
