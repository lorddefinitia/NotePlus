...
const TCHAR * Process::getStdout() const  {		return _stdoutStr.c_str();
	};

const TCHAR * Process::getStderr() const  {		return _stderrStr.c_str();
	};

int Process::getExitCode() const  {		return _exitCode;
	};

bool Process::hasStdout()  {		return (_stdoutStr.compare(TEXT("")) != 0);
	};

bool Process::hasStderr()  {		return (_stderrStr.compare(TEXT("")) != 0);
	};

WINAPI Process::staticListenerStdOut(void * myself) {		((Process *)myself)->listenerStdOut();
		return 0;
	};

WINAPI Process::staticListenerStdErr(void * myself)  {		((Process *)myself)->listenerStdErr();
		return 0;
	};
