...
HMENU PluginsManager::getMenuHandle()  {		return _hPluginsMenu;
	};

void PluginsManager::disable()  {_isDisabled = true;};
bool PluginsManager::hasPlugins() {return (_pluginInfos.size()!= 0);};
bool PluginsManager::inDynamicRange(int id)  { return _dynamicIDAlloc.isInRange(id); }
...
bool PluginsManager::isInLoadedDlls(const TCHAR *fn) const  {		for (size_t i = 0; i < _loadedDlls.size(); i++)
			if (generic_stricmp(fn, _loadedDlls[i].c_str()) == 0)
				return true;
		return false;
	};

void PluginsManager::addInLoadedDlls(const TCHAR *fn)  {		_loadedDlls.push_back(fn);
	};
