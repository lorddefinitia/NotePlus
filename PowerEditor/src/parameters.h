...
public:
    ...
    Date(const TCHAR *dateStr) { // timeStr should be Notepad++ date format : YYYYMMDD
        ...
        if (lstrlen(dateStr) == 8)
        {
            ...
            int y = generic_atoi((const char*)yyyy.c_str());
            int m = generic_atoi((const char*)mm.c_str());
            int d = generic_atoi((const char*)dd.c_str());
	...
	const TCHAR * getUserDefinedLangNameFromExt(TCHAR *ext, TCHAR *fullName) {
		...
		for (int i = 0 ; i < _nbUserLang ; i++)
		{
			...
			for (size_t j = 0 ; j < extVect.size() ; j++)
				if (!generic_stricmp(extVect[j].c_str(), ext) || (_tcschr((char*)fullName, '.') && !generic_stricmp(extVect[j].c_str(), fullName)))
