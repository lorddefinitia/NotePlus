...
TiXmlElementA::TiXmlElementA( const std::string& _value ) : TiXmlNodeA( TiXmlNodeA::ELEMENT ){
	firstChild = lastChild = 0;
	value = _value;
}
void TiXmlElementA::SetAttribute( const std::string& name, const std::string& _value )	{	
...
///< STL std::string form.
void TiXmlElementA::SetAttribute( const std::string& name, int _value )	 {	
	StringToBuffer n( name );
	if ( n.buffer )
...
TiXmlHandleA TiXmlHandleA::ChildElement( const char* value, int count ) const
{
 ...
 TiXmlBaseA::~TiXmlBaseA()					 {}
 void TiXmlBaseA::SetCondenseWhiteSpace( bool condense )		 { condenseWhiteSpace = condense; }
 bool TiXmlBaseA::IsWhiteSpaceCondensed()						 { return condenseWhiteSpace; }
int TiXmlBaseA::Row() const			 { return location.row + 1; }
int TiXmlBaseA::Column() const		 { return location.col + 1; }	///< See Row()
 bool	TiXmlBaseA::IsWhiteSpace( int c )		 { return ( isspace( c ) || c == '\n' || c == '\r' ); }
//const char * TiXmlAttributeA::Value() const  { return value.c_str (); }
void TiXmlAttributeA::SetValue(const char * _value)  { value = _value;}
 int TiXmlNodeA::Type() const	 { return type; }
bool TiXmlNodeA::NoChildren() const						 { return !firstChild; }
void  TiXmlNodeA::SetUserData( void* user )			 { userData = user; }
void TiXmlNodeA::CopyToClone( TiXmlNodeA* target ) const	 { target->SetValue (value.c_str() );												  target->userData = userData; }

TIXMLA_STRING TiXmlNodeA::SValue() const	 { return value ; }
void TiXmlAttributeA::SetName( const char* _name )	 { name = _name; }				///< Set the name of this attribute.
void TiXmlNodeA::SetValue( const char* _value )	 { value = _value; }				///< Set the value.
const char * TiXmlNodeA::Value() const { return value.c_str (); }
void TiXmlAttributeA::SetDocument( TiXmlDocumentA* doc )	 { document = doc; }
void TiXmlElementA::RemoveAttribute( const std::string& name )	 {	RemoveAttribute (name.c_str ());	}	///< STL std::string form.
 TiXmlCommentA::~TiXmlCommentA()	 {}
 TiXmlTextA::~TiXmlTextA()  {}
 TiXmlDeclarationA::~TiXmlDeclarationA()	 {}
const char * TiXmlDeclarationA::Version() const		 { return version.c_str (); }
const char * TiXmlDeclarationA::Encoding() const		 { return encoding.c_str (); }
const char * TiXmlDeclarationA::Standalone() const		 { return standalone.c_str (); }
 TiXmlUnknownA::~TiXmlUnknownA()  {}
 TiXmlDocumentA::~TiXmlDocumentA()  {}
bool TiXmlDocumentA::Error() const						 { return error; }
const char * TiXmlDocumentA::ErrorDesc() const	 { return errorDesc.c_str (); }
const int TiXmlDocumentA::ErrorId()	const				 { return errorId; }
int TiXmlDocumentA::ErrorRow()	 { return errorLocation.row+1; }
int TiXmlDocumentA::ErrorCol()	 { return errorLocation.col+1; }	///< The column where the error occured. See ErrorRow()
void TiXmlDocumentA::SetTabSize( int _tabsize )		 { tabsize = _tabsize; }
int TiXmlDocumentA::TabSize() const	 { return tabsize; }
void TiXmlDocumentA::ClearError()						 {	error = false; 												errorId = 0; 
												errorDesc = ""; 
												errorLocation.row = errorLocation.col = 0; 
												//errorLocation.last = 0; 
											}

void TiXmlDocumentA::Print() const						 { Print( stdout, 0 ); }
TiXmlHandleA TiXmlHandleA::FirstChild( const std::string& _value ) const			 { return FirstChild( _value.c_str() ); }
TiXmlHandleA TiXmlHandleA::FirstChildElement( const std::string& _value ) const		 { return FirstChildElement( _value.c_str() ); }
TiXmlHandleA TiXmlHandleA::Child( const std::string& _value, int index ) const			 { return Child( _value.c_str(), index ); }
TiXmlHandleA TiXmlHandleA::ChildElement( const std::string& _value, int index ) const	 { return ChildElement( _value.c_str(), index ); }
