...
//NOTE Manually added
void TiXmlNode::SetValue(const TCHAR * _value) { value = _value;}
// Internal Value function returning a TIXML_STRING
TIXML_STRING TiXmlNode::SValue() const	{ return value ; }
const TCHAR * TiXmlNode::Value() const { return value.c_str (); }
void TiXmlNode::CopyToClone( TiXmlNode* target ) const	{ 
  target->SetValue (value.c_str() );
  target->userData = userData;
}

void TiXmlAttribute::SetName( const TCHAR* _name )	{ name = _name; }				///< Set the name of this attribute.
void TiXmlAttribute::SetValue( const TCHAR* _value )	{ value = _value; }				///< Set the value.
void TiXmlAttribute::SetDocument( TiXmlDocument* doc )	{ document = doc; }

TiXmlDocument::~TiXmlDocument() {};
void TiXmlDocument::ClearError()	{	error = false; 
	errorId = 0; 
	errorDesc = TEXT(""); 
	errorLocation.row = errorLocation.col = 0; 
	//errorLocation.last = 0; 
}
bool TiXmlDocument::Error() const { return error; }
int TiXmlDocument::TabSize() const	{ return tabsize; }
...
bool TiXmlNode::NoChildren() const { return !firstChild; }
