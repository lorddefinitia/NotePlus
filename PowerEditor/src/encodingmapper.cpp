...
{
	size_t nbItem = sizeof(_encodings)/sizeof(EncodingUnit);
	if (index < 0 || (size_t)index >= nbItem)
		return -1;
	return _encodings[index]._codePage;
}
...
int EncodingMapper::getIndexFromEncoding(int encoding) const
{
	...
	for ( ; i < nbItem ; i++)
	{
		if (_encodings[i]._codePage == encoding)
		{
			found = true;
			break;
		}
...
int EncodingMapper::getEncodingFromString(const char *encodingAlias) const
{
	...
	{
		if (isInListA(encodingAlias, _encodings[i]._aliasList))
		{
			enc = _encodings[i]._codePage;
			break;
		}
	}
	return enc;
}static EncodingMapper * EncodingMapper::getInstance()  {return _pSelf;};
