...
BOOL CALLBACK FindReplaceDlg::run_dlgProc(UINT message, WPARAM wParam, 
                                          LPARAM lParam) {
	switch (message) 
	{
		case WM_INITDIALOG :
...
void FindIncrementDlg::addToRebar(ReBar * rebar) 
{
 ...
 int Searching::buildSearchFlags(const FindOption * option)  {
...
void Finder::init(HINSTANCE hInst, HWND hPere, ScintillaEditView **ppEditView)  {
		DockingDlgInterface::init(hInst, hPere);
		_ppEditView = ppEditView;
	};

void Finder::setFinderReadOnly(bool isReadOnly)  {
...
void FindReplaceDlg::gotoNextFoundResult(int direction = 0)  {if (_pFinder) _pFinder->gotoNextFoundResult(direction);};
void FindReplaceDlg::putFindResult(int result)  {
		_findAllResult = result;
	};

const TCHAR * FindReplaceDlg::getDir2Search() const  {return _env->_directory.c_str();};
void FindReplaceDlg::launchFindInFilesDlg()  {
...
bool FindReplaceDlg::isRecursive() const  { return _env->_isRecursive; };
bool FindReplaceDlg::isInHiddenDir() const  { return _env->_isInHiddenDir; };
...
//void FindIncrementDlg::setSearchText(const TCHAR * txt2find, bool)  {
//		::SendDlgItemMessage(_hSelf, IDC_INCFINDTEXT, WM_SETTEXT, 0, (LPARAM)txt2find);
//	};

void FindIncrementDlg::setSearchText(const TCHAR * txt2find, bool isUTF8 = false)  {
		if (!isUTF8)
		{
			::SendDlgItemMessage(_hSelf, IDC_INCFINDTEXT, WM_SETTEXT, 0, (LPARAM)txt2find);
			return;
...
FindStatus FindIncrementDlg::getFindStatus()  {
		return _FindStatus;
	}
